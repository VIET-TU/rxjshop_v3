...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	async handleConnection(socket: Socket) {
		...
		const ob$ = this.authService.send({ cmd: 'decode-jwt' }, { jwt })
	...
	private async createConversations(socket: Socket, userId: string) {
	...
	private async getFriendDetails(id: string) {
		...
		const activeFriend = await firstValueFrom(ob$).catch((err) => console.error(err))

		if (!activeFriend) return

		const friendsDetails = (await this.cache.get(
			`conversationUser ${activeFriend.id}`
		)) as unknown as { id: string; socketId: string } | undefined
	...
	async handleMessage(socket: Socket, newMessage: NewMessageDTO) {
		...
		const friendId = createdMessage.conversation.users.find((u) => u.id !== user.id).id
	...
	async declineCall(socket: Socket, friendId: string) {
		const friendDetails = await this.getFriendDetails(friendId)
	...
	async acceptCall(socket: Socket, friendId: string) {
		const friendDetails = await this.getFriendDetails(friendId)
